...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer,teacher,teacher_model=None):
    ...
    for idx, data in enumerate(train_loader):
        if epoch != idx // num_per_epoch + start_epoch:
            ...
            if cfg.BACKBONE.TRAIN_EPOCH == epoch:       #当轮数到一定值之后开始训练backbone
                ...
                optimizer, lr_scheduler = build_opt_lr(model.module, epoch) #获得新的优化器以及lr数值
                logger.info("model\n{}".format(describe(model.module)))
        ...
        if teacher:
            ...
            output = model(data,teacher_model)
...
def select_train(option):
    ...
    # load pretrain
        ...
        if option == True:
            model = load_pretrain(model, cfg.TRAIN.PRETRAINED,option='teacher').cuda()
        else:
            model = load_pretrain(model, cfg.TRAIN.PRETRAINED, option='student').cuda()
...
def main():
    ...
    teacher_model, _, __, ___, ____ = select_train(option)
    ...
    model, tb_writer, train_loader, optimizer, lr_scheduler = select_train(option=False)

    dist_model = DistModule(model)  # 加载预训练模型

    logger.info(lr_scheduler)
    logger.info("model prepare done")
    train(train_loader, dist_model, optimizer, lr_scheduler, tb_writer, teacher=False,teacher_model=teacher_model)
