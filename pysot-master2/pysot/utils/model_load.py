...
def load_pretrain(model, pretrained_path,option):
    ...
    if option != 'trained':
        for p in model_dict:
            if 'num_batches_tracked' in p:
                ...
                pp_dict[p] = model_dict[p]

        model_dict_plus = {}
        if option == 'teacher':
            for i ,pp in enumerate(pp_dict):
                if i >= 14:
                    model_dict_plus[pp] = pp_dict[pp]
                else:
                    pass
        elif option == 'student':
            for i ,pp in enumerate(pp_dict):
        ...
        for i ,pp in enumerate(model_dict_plus):
            for ii, pretrain in enumerate(pretrained_dict):
                if ii == i:
                    model_dict_plus[pp] = pretrained_dict[pretrain]
        pretrained_dict = model_dict_plus
